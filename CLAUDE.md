...
- **Login only accepts username, not email.** `POST /api/auth/token` (`backend/app/api/routes/auth.py`) uses `OAuth2PasswordRequestForm` and `authenticate_user()` looks up `User.username == username` only — there's no fallback to look up by `User.email`, even though `email` is captured at registration. `frontend/src/pages/LoginPage.tsx` only has a "Username" field to match. Fixing this needs a backend change (query by username OR email) before the frontend field can be relabeled/widened.
- **Registration only collects username/email/password.** `POST /api/auth/register` and `RegisterPage.tsx` have no fields beyond the three `User` columns that exist today (`username`, `email`, `password`/`hashed_password`). Adding more profile fields (name, etc.) needs both a `User` model/migration change and a form update — decide the actual field list before starting.
