...
**Auth**: bcrypt password hashing + JWT bearer tokens (`python-jose`), issued from `POST /api/auth/token` (`OAuth2PasswordRequestForm`) and validated per-request by `get_current_user`. `authenticate_user()` matches the submitted identifier against `User.username` OR `User.email`, so login accepts either — the OAuth2 form field is still literally named `username` (spec-mandated) even though it holds either value; `frontend/src/pages/LoginPage.tsx` labels it "Username or email" accordingly. The JWT `sub` claim always encodes the resolved `user.username` (never the raw login identifier), so `get_current_user`'s own `User.username == username` lookup doesn't need to know about email login at all. `User.full_name` is a required column collected at registration (`UserRegister.full_name` → `POST /api/auth/register`); `RegisterPage.tsx` also has a client-side-only "Confirm password" field with no backend counterpart.
...
Found while eyeballing the redesigned frontend against the real backend via `docker compose up --build` — none of these are visual, all require backend and/or frontend logic changes, so they're deliberately out of scope for a pure UI redesign:

- **No way to edit an experiment's details after creation.** `PATCH /api/experiments/{experiment_id}` already exists (`backend/app/api/routes/experiments.py`) and takes a full `Experiment` JSON body (unusual for this codebase — most other routes take query params, see the note on `POST /api/experiments/` above), but the handler only ever applies `experiment_details.name` and `.description` to the row — `hypothesis`, `unit_of_randomization`, `start_date`, `end_date`, and `status` are silently ignored even if sent. `OverviewPanel.tsx` is read-only (just a metadata `<dl>` and a delete button). Fixing "edit hypothesis/start date/etc." needs the backend handler updated to apply those fields first, then an edit form/modal on the frontend.
- **Planning and Analysis require typing raw metric/variant IDs.** `PlanningPanel.tsx` and `AnalysisPanel.tsx` both have a plain "Metric ID" number `Field`, and `AnalysisPanel.tsx` additionally has separate "Variant A/B successes/total" number fields instead of picking real variants — this was called out above as a known simplification ("built before the metrics-list endpoint was fixed"), but it's a real usability gap: a user has to already know or look up numeric IDs. `listMetrics(experimentId)`/`listVariants(experimentId)` (`frontend/src/api/experiments.ts`) already exist and are used elsewhere (`MetricsPanel.tsx`, `VariantsPanel.tsx`), so wiring both panels' ID fields into `Select` dropdowns populated from those is a frontend-only fix — no backend change needed.
