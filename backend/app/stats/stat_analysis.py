...
def calculate_srm(p1: float, p2: float, n1: int, n2: int, alpha: float = 0.05):
    """
    Calculate the Sample Ratio Mismatch (SRM) between two groups.
    ...
    Returns:
        bool: True if there is a significant sample ratio mismatch, False otherwise.
    """
    ...
    # Calculate the critical chi-squared value for the given alpha and degrees of freedom (df=1)
    critical_chi_squared = stats.chi2.ppf(1 - alpha, df=1)
    
    # Check if the chi-squared statistic exceeds the critical value
    return chi_squared_stat > critical_chi_squared
